  ...
  <div>
    <div class="form-row">
      <div class="col">
        ...
        <input type="date" id="txt_fechanacimiento" name="txt_fechanacimiento" aria-describedby="ayuda_fecha_nacimiento" class="form-control" v-model="formData.fechadenacimiento" placeholder="Fecha de Nacimiento">
      ...
      <div class="col">
        ...
        <input type="text" id="txt_residencia" name="txt_residencia" aria-describedby="ayuda_residencia" class="form-control" v-model="formData.recidencia" placeholder="Dirección de residencia">
...
export default {
  ...
  methods:{
    async obtenerDatos(){
      try {
        const reponse = await axios.get('http://localhost:3000/usuario');
    ...
    calcular_edad(){
      ...
      const objetoNacimiento = new Date(this.formData.fechadenacimiento);
      ...
      if (dias < 0) {
        meses--;
        const ultimodiadelmesanterior = new Date(ahora.getFullYear(),ahora.getMonth(),0).getDate();
  ...
  mounted(){
    //Opcionalmente se obtienen los datos cuando el componente es creado, sin necesidad de presionar el botón
    //this.obtenerDatos();
  } 
}
</script>
